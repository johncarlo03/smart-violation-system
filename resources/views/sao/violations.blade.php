                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 mb-8">
                    <div class="bg-white rounded-2xl p-6 shadow-md border border-gray-100">
                        <div class="flex items-center justify-between">
                            ...
                            <div
                                class="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-indigo-100 flex items-center justify-center">
                                <svg class="w-6 h-6 sm:w-7 sm:h-7 text-indigo-600" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-md border border-gray-100">
                        <div class="flex items-center justify-between">
                            <div>
                                <div class="text-xs font-black uppercase tracking-[0.2em] text-gray-400">Pending Review
                                </div>
                                <div class="text-3xl sm:text-4xl font-black text-orange-500 mt-2">{{ $pendingReview }}</div>
                            </div>
                            <div
                                class="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-orange-100 flex items-center justify-center">
                                <svg class="w-6 h-6 sm:w-7 sm:h-7 text-orange-600" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-md border border-gray-100">
                        <div class="flex items-center justify-between">
                            <div>
                                <div class="text-xs font-black uppercase tracking-[0.2em] text-gray-400">Major Incidents
                                </div>
                            ...
                            <div class="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-red-100 flex items-center justify-center">
                                <svg class="w-6 h-6 sm:w-7 sm:h-7 text-red-600" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div
                                ...
                                <div class="flex items-center gap-3 mb-4">
                                    @if($v->student->profile_photo)
                                        <img src="{{ asset('storage/' . $v->student->profile_photo) }}"
                                            class="w-10 h-10 rounded-full object-cover shadow-sm">
                                    @else
                                        <div
                                            class="w-10 h-10 rounded-full {{ $v->department->badgeColor ?? 'bg-gray-100' }} flex items-center justify-center font-black text-sm shadow-sm">
                                            {{ strtoupper(substr($v->student->name, 0, 1)) }}
                                        </div>
                                    @endif
                                    <div class="flex-1">
                                        <div class="font-bold text-gray-900 text-sm leading-tight">{{ $v->student->name }}
                                        </div>
                                    ...
                                    <span
                                        class="px-2 py-1 rounded-full text-[9px] font-black uppercase border {{ $v->status === 'pending' ? 'bg-orange-100 text-orange-700 border-orange-200' : 'bg-green-100 text-green-700 border-green-200' }}">
                                        {{ $v->status }}
                                    </span>
                                </div>
                                ...
                                <div class="bg-gray-50 rounded-xl p-3 mb-4">
                                    ...
                                    <div class="text-[9px] text-gray-500 mt-1 uppercase tracking-widest font-bold">
                                        {{ $v->offense->category ?? '' }}</div>
                                </div>

                                <div class="flex items-center justify-between pt-2 border-t border-gray-50">
                                    <div class="text-[10px] text-gray-400">Reported By: <span
                                            class="text-gray-600 font-medium">{{ $v->cso->name }}</span></div>
                                    <div class="flex gap-2">
                                        ...
                                        @endif
                                        <a href="#"
                                            class="px-4 py-2 bg-indigo-600 text-white text-xs font-bold rounded-lg shadow-sm">Manage</a>
                                    </div>
                                </div>
                            </div>
                        @empty
                                    ...
                                    <tr class="hover:bg-indigo-50/40 transition duration-200">
                                        <td class="px-6 py-5">
                                            <div class="flex items-center gap-4">
                                                ...
                                                @else
                                                    <div
                                                        class="w-12 h-12 rounded-full {{ $v->department->badgeColor ?? 'bg-gray-100' }} flex items-center justify-center font-black text-lg shadow-sm">
                                                        {{ strtoupper(substr($v->student->name, 0, 1)) }}
                                                    </div>
                                                @endif
                                                <div>
                                        ...
                                        <td class="px-6 py-5 text-right">
                                            <div class="flex justify-end gap-2">
                                                @if($v->evidence_image)
                                                    <button onclick="openEvidenceModal('{{ $v->evidence_image }}')"
                                                        class="inline-flex items-center px-3 py-2 rounded-xl bg-blue-100 text-blue-700 text-xs font-bold hover:bg-blue-200 transition">View
                                                        Photo</button>
                                                @endif
                                                <a href="#"
                                                    class="inline-flex items-center px-3 py-2 rounded-xl bg-indigo-600 text-white text-xs font-bold hover:bg-indigo-700 shadow-sm transition">Manage</a>
                                            </div>
                                        </td>
                                    </tr>
                                @empty
